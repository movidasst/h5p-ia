...
frame = Path('h5p-viewer-frame.html')
s = frame.read_text(encoding='utf-8')
if "customJs:'/h5p-compat.js'" not in s:
    needle = "reportingIsEnabled:true\n        });"
    repl = "reportingIsEnabled:true,\n          customJs:'/h5p-compat.js'\n        });"
    if needle not in s:
        raise SystemExit('viewer options anchor not found')
    s = s.replace(needle, repl, 1)
    frame.write_text(s, encoding='utf-8')
...
'''
Path('h5p-compat.js').write_text(compat, encoding='utf-8')
print('H5P ContentType compatibility applied')
